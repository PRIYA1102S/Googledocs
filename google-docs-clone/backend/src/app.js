...
io.on("connection", (socket) => {
  ...
  socket.on("document-change", async ({ documentId, delta, content }) => {
    try {
      logger.info(
        `Document change received from ${socket.userName} (${socket.userId}) for document ${documentId}`
      );

      // Check if user has edit permissions
      if (
        socket.userPermission !== "owner" &&
        socket.userPermission !== "editor"
      ) {
        socket.emit("error", { message: "Unauthorized to edit document" });
        logger.warn(
          `User ${socket.userName} (${socket.userId}) attempted to edit document ${documentId} without permission`
        );
        return;
      }
  ...
  socket.on("disconnect", () => {
    if (socket.documentId && documentRooms.has(socket.documentId)) {
      ...
      logger.info(
        `User ${socket.userName} (${socket.userId}) left document ${socket.documentId}`
      );
    }
